...
function formatMoney(amount) {
   ...
   * formatMoney(1) => "£1.00"
   */
  //let formatGB = new Intl.NumberFormat('en-GB', {
...
function calculateCircleArea(r) {
  ...
  return Math.round(Math.PI * r * r * 1000) / 1000;
}
...
function capitaliseString(str) {
  ...
  let last = str.toLowerCase().slice(1);
  return first + last;
}
...
function dontShoutSentence(SENTENCE) {
  ...
  let rest = SENTENCE.toLowerCase().slice(1);
  return first + rest;
...
function getMiddle(str) {
  ...
  let pos = Math.floor(str.split('').length / 2);
  return strLen % 2 === 0 ? str.slice(pos -1, pos + 1) : str.charAt(pos);
}
...
function convertToCamelCase(words) {
  ...
  if (words.split(' ').length === 1) return words.toLowerCase();
  let camelCased = words.split(' ')
  .map(word => word[0].toUpperCase() + word.substring(1))
  .join('');
  let str = camelCased.charAt(0).toLowerCase() + camelCased.slice(1);
  return str;
}
...
function isItemOmnipresent(arrayOfArrays, item) {
  ...
  return arrayOfArrays.forEach(arr => arr.includes(item));
}
...
function createNorthcoder(name, yearOfBirth) {
  ...
  const coder = {};
  coder['name'] = name;
  coder['age'] = 2019 - yearOfBirth;
  coder['language'] = 'JavaScript';
  return coder;
}
...
function getNorthcodersNames(northcoders) {
  ...
  if (northcoders.length === 0) return [];
  const names = [];
